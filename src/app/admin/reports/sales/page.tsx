...
export default function SalesReportPage() {
  ...
  const generateReport = () => {
    ...
    const doc = generatePDFReport({
      ...
      totals: [
        { label: 'Total Revenue', value: `P ${totalRevenue.toFixed(2)}` },
        { label: 'Total Transactions', value: totalTransactions.toString() },
        { label: 'Average Sale', value: `P ${averageSale.toFixed(2)}` },
      ],
    });
                ...
                <tbody>
                  {salesData.map((sale) => (
                    <tr key={sale.id} className="border-b">
                      ...
                      <td className="p-3 font-medium">P{Number(sale.total).toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
